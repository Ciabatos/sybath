...
import { TPlayerGearInventoryParams } from "@/db/postgresMainDatabase/schemas/inventory/playerGearInventory"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchPlayerGearInventoryService(parsedParams, { clientEtag })
