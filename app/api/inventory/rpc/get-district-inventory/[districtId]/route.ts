...
import { getDistrictInventory, TDistrictInventoryParams } from "@/db/postgresMainDatabase/schemas/inventory/districtInventory"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams } ): Promise<NextResponse> {
  ...
  try {
    if (!cachedData || Date.now() - lastUpdated > CACHE_TTL) {
      cachedData = await getDistrictInventory(parsedParams)
