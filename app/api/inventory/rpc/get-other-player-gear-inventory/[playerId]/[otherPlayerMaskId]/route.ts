...
import { TOtherPlayerGearInventoryParams } from "@/db/postgresMainDatabase/schemas/inventory/otherPlayerGearInventory"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchOtherPlayerGearInventoryService(parsedParams, {
      clientEtag,
    })
