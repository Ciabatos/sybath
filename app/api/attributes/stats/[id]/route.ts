...
import { getAttributesStatsByKey, TAttributesStatsParams } from "@/db/postgresMainDatabase/schemas/attributes/stats"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }  ): Promise<NextResponse> {
  ...
  try {
    const result = await getAttributesStatsByKey(parsedParams)
