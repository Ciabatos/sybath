...
import { TOtherPlayerStatsParams } from "@/db/postgresMainDatabase/schemas/attributes/otherPlayerStats"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchOtherPlayerStatsService(parsedParams, { clientEtag })
