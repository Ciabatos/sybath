...
import { TOtherPlayerAbilitiesParams } from "@/db/postgresMainDatabase/schemas/attributes/otherPlayerAbilities"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchOtherPlayerAbilitiesService(parsedParams, { clientEtag })
