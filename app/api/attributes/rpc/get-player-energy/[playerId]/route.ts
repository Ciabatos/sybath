...
import { TPlayerEnergyParams } from "@/db/postgresMainDatabase/schemas/attributes/playerEnergy"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchPlayerEnergyService(parsedParams, {
