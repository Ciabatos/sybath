...
import { TAllSkillsParams } from "@/db/postgresMainDatabase/schemas/attributes/allSkills"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchAllSkillsService(parsedParams, {
