...
import { TTradeInventoryParams } from "@/db/postgresMainDatabase/schemas/trade/tradeInventory"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchTradeInventoryService(parsedParams, {
