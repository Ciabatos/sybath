...
import { getDistrictsDistrictTypesByKey, TDistrictsDistrictTypesParams } from "@/db/postgresMainDatabase/schemas/districts/districtTypes"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }  ): Promise<NextResponse> {
  ...
  try {
    const result = await getDistrictsDistrictTypesByKey(parsedParams)
