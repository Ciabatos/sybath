...
import { TWorldMapTilesMapRegionsParams } from "@/db/postgresMainDatabase/schemas/world/mapTilesMapRegions"
...
}) satisfies z.ZodType<TWorldMapTilesMapRegionsParams>
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchWorldMapTilesMapRegionsByKeyService(parsedParams, {
      clientEtag,
    })
