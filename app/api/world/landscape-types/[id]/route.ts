...
import { getWorldLandscapeTypesByKey, TWorldLandscapeTypesParams } from "@/db/postgresMainDatabase/schemas/world/landscapeTypes"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  ...
  try {
    const result = await getWorldLandscapeTypesByKey(parsedParams)
