...
import { getWorldMapsByKey, TWorldMapsParams } from "@/db/postgresMainDatabase/schemas/world/maps"
...
const typeParamsSchema = z.object({
  id: z.coerce.number(),
}) satisfies z.ZodType<TWorldMapsParams>
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  ...
  try {
    const result = await getWorldMapsByKey(parsedParams)
