...
import { TPlayersOnTheSameTileParams } from "@/db/postgresMainDatabase/schemas/world/playersOnTheSameTile"
...
}) satisfies z.ZodType<TPlayersOnTheSameTileParams>
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchPlayersOnTheSameTileService(parsedParams, { clientEtag })
