...
import { TKnownMapTilesResourcesOnMapParams } from "@/db/postgresMainDatabase/schemas/world/knownMapTilesResourcesOnMap"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchKnownMapTilesResourcesOnMapService(parsedParams, {
