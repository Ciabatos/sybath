...
import { TKnownMapTilesResourcesOnTileParams } from "@/db/postgresMainDatabase/schemas/world/knownMapTilesResourcesOnTile"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchKnownMapTilesResourcesOnTileService(parsedParams, {
