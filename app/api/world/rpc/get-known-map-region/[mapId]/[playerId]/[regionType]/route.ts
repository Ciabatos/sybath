...
import { TKnownMapRegionParams } from "@/db/postgresMainDatabase/schemas/world/knownMapRegion"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchKnownMapRegionService(parsedParams, { clientEtag })
