...
import { getWorldTerrainTypesByKey, TWorldTerrainTypesParams } from "@/db/postgresMainDatabase/schemas/world/terrainTypes"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  ...
  try {
    const result = await getWorldTerrainTypesByKey(parsedParams)
