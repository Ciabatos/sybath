...
import { getCitiesCitiesByKey, TCitiesCitiesParams } from "@/db/postgresMainDatabase/schemas/cities/cities"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  ...
  try {
    const result = await getCitiesCitiesByKey(parsedParams)
