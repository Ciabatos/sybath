...
import { TPlayerCityParams } from "@/db/postgresMainDatabase/schemas/cities/playerCity"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchPlayerCityService(parsedParams, { clientEtag })
