...
import { TPlayerRecipesParams } from "@/db/postgresMainDatabase/schemas/items/playerRecipes"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchPlayerRecipesService(parsedParams, {
