...
import { getItemsItemStatsByKey, TItemsItemStatsParams } from "@/db/postgresMainDatabase/schemas/items/itemStats"
...
const typeParamsSchema = z.object({
  id: z.coerce.number(),
}) satisfies z.ZodType<TItemsItemStatsParams>
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }  ): Promise<NextResponse> {
  ...
  try {
    const result = await getItemsItemStatsByKey(parsedParams)
