...
import { TItemsRecipeMaterialsParams } from "@/db/postgresMainDatabase/schemas/items/recipeMaterials"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchItemsRecipeMaterialsByKeyService(parsedParams, {
