...
import { TSquadInvitesParams } from "@/db/postgresMainDatabase/schemas/squad/squadInvites"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchSquadInvitesService(parsedParams, {
