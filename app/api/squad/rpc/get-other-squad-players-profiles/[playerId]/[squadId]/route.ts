...
import { TOtherSquadPlayersProfilesParams } from "@/db/postgresMainDatabase/schemas/squad/otherSquadPlayersProfiles"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchOtherSquadPlayersProfilesService(parsedParams, {
      clientEtag,
    })
