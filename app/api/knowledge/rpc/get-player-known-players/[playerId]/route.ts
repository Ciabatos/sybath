...
import { TPlayerKnownPlayersParams } from "@/db/postgresMainDatabase/schemas/knowledge/playerKnownPlayers"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchPlayerKnownPlayersService(parsedParams, {
