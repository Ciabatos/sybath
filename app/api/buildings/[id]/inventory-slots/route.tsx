...
import { getBuildingInventorySlots } from "@/db/postgresMainDatabase/schemas/items/inventories"
...
export async function GET(request: NextRequest, { params }: { params: TypeParams }): Promise<NextResponse> {
  ...
  const buildingId = parsedParamsZod.id
  // const searchQueryParams = request.nextUrl.searchParams
  // const login = searchQueryParams.get("login")

  try {
    const result = await getBuildingInventorySlots(sessionPlayerId, buildingId)
