...
import { TOtherPlayerProfileParams } from "@/db/postgresMainDatabase/schemas/players/otherPlayerProfile"
...
export async function GET(request: NextRequest, { params }: { params: TApiParams }): Promise<NextResponse> {
  try {
    ...
    const { record, etag, cacheHit, etagMatched } = await fetchOtherPlayerProfileService(parsedParams, { clientEtag })
