DROP FUNCTION players.get_other_player_knowledge_requests(int4);

CREATE OR REPLACE FUNCTION knowledge.get_other_player_knowledge_requests(p_player_id integer)
 RETURNS TABLE(other_player_knowledge_request_id integer, other_player_id text, name character varying, second_name character varying, nickname character varying, image_portrait character varying, knowledge_type_id integer, created_at timestamp without time zone)
 LANGUAGE plpgsql
AS $function$
BEGIN
...
COMMENT ON FUNCTION knowledge.get_other_player_knowledge_requests(int4) IS 'get_api';
