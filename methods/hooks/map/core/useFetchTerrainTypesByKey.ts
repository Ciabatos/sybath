...
"use client"
import { TMapTerrainTypesParams, TMapTerrainTypesRecordById } from "@/db/postgresMainDatabase/schemas/map/terrainTypes"
...
export function useFetchTerrainTypesByKey(params: TMapTerrainTypesParams) {
  const terrainTypes = useAtomValue(terrainTypesAtom)
  const setTerrainTypes = useSetAtom(terrainTypesAtom)

  const { data } = useSWR(`/api/map/terrain-types/${params.id}`, { refreshInterval: 3000 })
