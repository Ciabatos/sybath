...
"use client"
import { TInventoryInventorySlotTypesRecordById, TInventoryInventorySlotTypes, TInventoryInventorySlotTypesParams } from "@/db/postgresMainDatabase/schemas/inventory/inventorySlotTypes"
...
export function useFetchInventoryInventorySlotTypesByKey( params: TInventoryInventorySlotTypesParams ) {
  ...
  const { data } = useSWR<TInventoryInventorySlotTypes[]>(`/api/inventory/inventory-slot-types/${params.id}`, { refreshInterval: 3000 })
