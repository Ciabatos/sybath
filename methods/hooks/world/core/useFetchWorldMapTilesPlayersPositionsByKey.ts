...
"use client"
import { TWorldMapTilesPlayersPositionsRecordByMapIdMapTileXMapTileY, TWorldMapTilesPlayersPositionsParams } from "@/db/postgresMainDatabase/schemas/world/mapTilesPlayersPositions"
...
export function useFetchWorldMapTilesPlayersPositionsByKey( params: TWorldMapTilesPlayersPositionsParams ) {
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(prevDataRef.current) !== JSON.stringify(data)) {
      const index = data ? (arrayToObjectKey(["mapId", "mapTileX", "mapTileY"], data) as TWorldMapTilesPlayersPositionsRecordByMapIdMapTileXMapTileY) : {}
