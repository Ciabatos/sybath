...
export async function getPlayerStatsServer( params: TPlayerStatsParams): Promise<{
  ...
  const data = getPlayerStatsData ? (arrayToObjectKey(["statId"], getPlayerStatsData) as TPlayerStatsRecordByStatId) : {}

  const result = { raw: getPlayerStatsData, byKey: data, apiPath: `/api/attributes/rpc/get-player-stats/${params.playerId}`, atomName: `playerStatsAtom`  }
