...
export async function getAttributesPlayerSkillsByKeyServer(params: TAttributesPlayerSkillsParams): Promise<{
  ...
  const data = getAttributesPlayerSkillsByKeyData
    ? (arrayToObjectKey(["playerId"], getAttributesPlayerSkillsByKeyData) as TAttributesPlayerSkillsRecordByPlayerId)
    : {}

  return {
    raw: getAttributesPlayerSkillsByKeyData,
    byKey: data,
    apiPath: `/api/attributes/player-skills/${params.playerId}`,
  }
}
