...
export async function getBuildingInventoryServer( params: TBuildingInventoryParams): Promise<{
  ...
  const data = getBuildingInventoryData ? (arrayToObjectKey(["slotId"], getBuildingInventoryData) as TBuildingInventoryRecordBySlotId) : {}

  const result = { raw: getBuildingInventoryData, byKey: data, apiPath: `/api/inventory/rpc/get-building-inventory/${params.buildingId}`, atomName: `buildingInventoryAtom`  }
