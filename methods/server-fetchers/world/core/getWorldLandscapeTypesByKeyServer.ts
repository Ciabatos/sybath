...
export async function getWorldLandscapeTypesByKeyServer(params: TWorldLandscapeTypesParams): Promise<{
  ...
  const data = getWorldLandscapeTypesByKeyData
    ? (arrayToObjectKey(["id"], getWorldLandscapeTypesByKeyData) as TWorldLandscapeTypesRecordById)
    : {}

  return {
    raw: getWorldLandscapeTypesByKeyData,
    byKey: data,
