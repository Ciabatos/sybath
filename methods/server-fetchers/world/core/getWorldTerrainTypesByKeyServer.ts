...
export async function getWorldTerrainTypesByKeyServer(params: TWorldTerrainTypesParams): Promise<{
  ...
  const data = getWorldTerrainTypesByKeyData
    ? (arrayToObjectKey(["id"], getWorldTerrainTypesByKeyData) as TWorldTerrainTypesRecordById)
    : {}

  return {
    raw: getWorldTerrainTypesByKeyData,
    byKey: data,
