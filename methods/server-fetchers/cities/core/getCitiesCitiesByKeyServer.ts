...
export async function getCitiesCitiesByKeyServer(params: TCitiesCitiesParams): Promise<{
  ...
  const data = getCitiesCitiesByKeyData
    ? (arrayToObjectKey(["mapTileX", "mapTileY"], getCitiesCitiesByKeyData) as TCitiesCitiesRecordByMapTileXMapTileY)
    : {}

  return {
    raw: getCitiesCitiesByKeyData,
    byKey: data,
