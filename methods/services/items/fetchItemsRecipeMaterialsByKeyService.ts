...
import { getItemsRecipeMaterialsByKey } from "@/db/postgresMainDatabase/schemas/items/recipeMaterials"
...
export async function fetchItemsRecipeMaterialsByKeyService(
  params: TItemsRecipeMaterialsParams,
  options?: { clientEtag?: string; forceFresh?: boolean },
): Promise<TFetchResult> {
  const cacheKey = makeCacheKey("getItemsRecipeMaterialsByKey", params)
  ...
  const raw = await getItemsRecipeMaterialsByKey(params)
