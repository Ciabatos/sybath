...
export async function fetchItemsRecipeMaterialsService(options?: {
  clientEtag?: string
  forceFresh?: boolean
}): Promise<TFetchResult> {
  const cacheKey = makeCacheKey("getItemsRecipeMaterials")
  ...
  const raw = await getItemsRecipeMaterials()
